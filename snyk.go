...
func drawhelpscreen() { // MARK: drawhelpscreen
	...
	rl.DrawText("help", (monw/2)-(length/2)+rInt(-5, 6), 100+rInt(-5, 6), 120, randomcolor())
	rl.DrawText("help", (monw/2)-(length/2), 100, 120, rl.Black)

	length = rl.MeasureText("arrow keys move", 60)
	rl.DrawText("arrow keys move", (monw/2)-(length/2), 150, 60, rl.Black)
	length = rl.MeasureText("don't hit the walls", 60)
	rl.DrawText("don't hit the walls", (monw/2)-(length/2), 200, 60, rl.Black)
...
func setres(w, h int) { // MARK: setres
	...
	if monw > 1500 && monw < 1800 {
		...
		camera.Offset.X = float32(0 - (monh / 5))
		camera.Offset.Y = float32(0 - (monh / 5))
		camera.Zoom = 1.0
	} else if monw >= 1800 && monw < 2000 {
		camera.Target.X = float32(monw / 2)
		camera.Target.Y = float32(monh / 2)
		camera.Offset.X = float32(monw / 2)
		camera.Offset.Y = float32(monh / 2)
		camera.Zoom = 1.5
		...
		camera.Offset.X = float32(0 - (monh / 3))
		camera.Offset.Y = float32(10 - (monh/7 + 20))
		camera.Zoom = 2.8
	}
}
